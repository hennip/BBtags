...
M2<-"model{
  ...
  for(t in 1:T){
    ...
    surv_sea[j]=exp(-(M/52+h[j]+F_sea/11))  # survival based on instantenous mortality rates per week
                                                # M: natural mortality, h: trap induced mortality
                                                # F: fishing mortality
                                                
    harv_sea[j]=keep_tag*(1-surv_sea[j])*(F_sea/11)/(M/52+F_sea/11+h[j]) # harvest based on inst. mortalities
    
    surv_river[j]=exp(-(M/52+F_river/12+h[j]))
    harv_river[j]=keep_tag*(1-surv_river[j])*(F_river/12)/(M/52+F_river/12+h[j])
  ...
  #handling_mort~dbeta(27,132) # from Siira et al 
  handling_mort~dbeta(1.8,7.2) # Ruokonen et al 2021
  loose_tag~dbeta(12,68)  # from Siira et al, over three months
  ...
  loose_tagP~dbeta(12,68) 
  #handling_mortP~dbeta(27,132)
  handling_mortP~dbeta(1.8,7.2) # meidan papru 2021
